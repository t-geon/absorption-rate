...
%과제에서 inv를 사용해 제가 구한 값과 비교하라고 했는데 보고서에 쓰는건지 코드로 구현하는건지 몰라서 코드로도 작성했습니다.
for id=1:20%x와 xx의 값을 접근하기 위한 반복문으로 입니다.
    if(x(id)==xx(id))%slv를 이용해 구한 x이 값과 inv함수를 사용해 구한 xx의 값이 같다면 if문 안으로 들어갑니다(이 동작을 통해 x값을 정확히 구했는지 알 수 있습니다)
        disp("같습니다");%x와 xx가 같다면 "같습니다" 를 출력합니다
    else%slv를 이용해 구한 x와 inv함수를 이용해 구한 xx의 값이 다르다면 if문 안으로 들어갑니다
        disp("다릅니다");%x와 xx가 다르다면 "다릅니다"를 출력합니다
    end%if(x(id)==xx(id))와  else를 끝냅니다.
end%for id=1:20를 끝냅니다.